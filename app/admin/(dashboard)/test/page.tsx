...
export default async function TestPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Panel>
        {bots.length ? <BotTester bots={bots.map((bot) => ({ id: bot.id, slug: bot.slug, name: bot.name }))} /> : <EmptyState title="No active bots" body="Create and activate a bot before testing chat." />}
      </Panel>
    </div>
  );
}
